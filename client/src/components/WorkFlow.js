...
const WorkFlow = (props) => {
    ...
    const ckForComplete = (stepNum, taskNum) => {
        ...
        let counter = 0;
        if (tempTaskObj[stepNum].tasks[taskNum].indexOf(":not-complete") !== -1) {
            tempTaskObj[stepNum].tasks[taskNum] = tempTaskObj[stepNum].tasks[taskNum].replace(":not-complete", ":complete");
        } else {
            tempTaskObj[stepNum].tasks[taskNum] = tempTaskObj[stepNum].tasks[taskNum].replace(":complete", ":not-complete");
        }

        let taskListObj = [];
        for (let i = 0; i < tempTaskObj[stepNum].tasks.length; i++) {
            taskListObj.push(tempTaskObj[stepNum].tasks[i]);
            if (tempTaskObj[stepNum].tasks[i].indexOf(":not-complete") === -1) {
                counter = counter + 1;
            }
        }
            ...
            <div className="row">
                <div className="col-md-12">
                    <select className="form-control" name="ticketList" onChange={() => populateFields()}>
                        ...
                        <option value="test-ticket-1">test-ticket-1</option>
                        <option value="test-ticket-2">test-ticket-2</option>
                        <option value="test-ticket-3">test-ticket-3</option>
                    </select>
                </div>
                <div className="col-md-12">
                    ...
                    stepsData.map((step, i) => {
                        ...
                        return (
                            <div className="col" key={i} data-step={i}>
                                <div className={step.tasks.toString().indexOf(":not-complete") === -1 ? "card mb-4 rounded-3 shadow-sw alert-success" : "card mb-4 rounded-3 shadow-sw alert-secondary"}
                                            ...
                                            <div data-steplist={i} >
                                                <ul className="list-group mt-3 mb-4" data-step={step.stepTitle}>
                                                    ...
                                                    {(typeof step.tasks) === "object" && step.tasks.length > 0 ? step.tasks.map((task, j) => {
                                                        if (task.indexOf(":complete") !== -1 || task.indexOf(":not-complete") !== -1) {
                                                            return (<button key={j} className={task.indexOf(":not-complete") !== -1 ? "list-group-item list-group-item-action list-group-item-danger" : "list-group-item list-group-item-action list-group-item-onfo"} data-step={step.stepTitle}
                                                                name={task.substring(0, task.indexOf(":"))} onClick={() => ckForComplete(i, j)}>{task.substring(0, task.indexOf(":"))}</button>)
                                                        }
                                                    }) : null}
                                                </ul>
                                            </div>
                                            ...
                                            <ul className="list-group">
                                                {(typeof activeTaskList) === "object" ? activeTaskList.map((task, j) => {
                                                    return (<li className="list-group-item list-group-item-secondary" key={j} data-remove={task} >{task} <i className="fas fa-trash pointer"
                                                        key={i} onClick={() => removeTask(j)}></i></li>)
                                                }) : null}
